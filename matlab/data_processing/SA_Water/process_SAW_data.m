...
filename = 'RIVER MURRAY - UNI WA - Brendan Bucsh 2020.xlsx';
        ...
        for j = 1:length(u_vars)
            ...
            if ~isempty(s_var)
                ...
                if strcmpi(nVar,'Ignore') == 0
                    ...
                    if ~isempty(sss)
                        if nConv > 0
                            saw_2020.(nSite).(nVar).Data = rdata(ss(sss)) .* nConv;
                        else
                            saw_2020.(nSite).(nVar).Data = conductivity2salinity(rdata(ss(sss)));
                        end
                        saw_2020.(nSite).(nVar).Date = mdate(ss(sss));
                        saw_2020.(nSite).(nVar).Depth(1:length(sss),1) = 0;
                        saw_2020.(nSite).(nVar).X = nX;
                        saw_2020.(nSite).(nVar).Y = nY;
...
saw_2020 = merge_sites(saw_2020);




save saw_2020.mat saw_2020 -mat


summerise_data('saw_2020.mat','saw_2020/');
